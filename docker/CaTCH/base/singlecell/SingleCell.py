...
from CaTCH.base.Exceptions import InvalidCellRecordException, BarcodeMismatchException
...
single 10X cell barcode and one or more CaTCH barcodes with their respective
UMIs. 
"""
...
class SingleCell:
    ...
    def merge(self, other: SingleCell):
        ...
        for catch_bc in other._catch_umi_rel.keys():
            if not self._catch_umi_rel.get(catch_bc):
                ...
                self._catch_umi_rel[catch_bc].extend(other._catch_umi_rel[catch_bc].copy())
    ...
    def collapseCaTCHBarcodes(self, maxDist: int = 2, inplace: bool = True) -> SingleCell:
        """
        Collapses the CaTCH barcodes of the current cell. In brief, the method calculates the
        Hamming distance between all pairs of CaTCH barcodes and merges those with the distance
        smaller than the cut-off value maxDist into a single CaTCH barcode, while keeping all
        UMIs.
        """
        counts = dict()
        nTotalReads = 0
