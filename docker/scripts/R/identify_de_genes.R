...
### Pathway analysis
# Rebuild a Seurat object from the combined pseudobulk counts of all SCE files
# so the pathway analysis below sees every condition as well.
pseudo <- CreateSeuratObject(
    counts = as.sparse(pb$counts),
    meta.data = pb$metadata %>% column_to_rownames("Sample")
)
