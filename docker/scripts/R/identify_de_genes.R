...
for (t in setdiff(levels(sce$Condition), ref.Condition)) {
    ...
    deres <- FindMarkers(object = pseudo, 
                           ident.1 = t, 
                           ident.2 = ref.Condition,
                           test.use = "DESeq2")
    
    reslist[[contrast_name]] <- deres
    write.table(deres, gzfile(paste("DE_", "Seurat_", contrast_name, "_GENES_all.tsv.gz", sep = "")), sep = "\t", row.names = FALSE, quote = F)
        
    pdf(
        file = paste("DE_", "Seurat_", contrast_name, "_GENES_Volcano_p", opt$pcut, "_lfc", opt$fcut, ".pdf", sep = ""),
        width = 15, height = 10
    )
    print(EnhancedVolcano(deres,
                          ...
                          legendIconSize = 8.0,
                          drawConnectors = TRUE,
                          widthConnectors = 0.25) + coord_flip()
          )
    dev.off()
