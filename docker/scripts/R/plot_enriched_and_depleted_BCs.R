...
option_list = list(
  ...
  make_option(c("--baseCond"), type="character", default = NULL, 
  ...
  make_option(c("--height"), type = "numeric", default = 25, 
...
for (t in setdiff(levels(metadata$Condition), ref.Condition)) {
    ...
    if (nrow(r) > 1){
        ...
        pdf(file = paste("DE", "DESEQ2", contrast_name, "BCs_Volcano", "p005_lfc15.pdf", sep = "_"), 
             width = 15, height = 10)
...
save(comparison_objs, file = paste0(opt$out,'DE_list.rda.gz'), compress = "gzip")
rm(comparison_objs)
