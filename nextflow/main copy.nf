...
Channel
    .fromPath(params.libraries)
    .set { Ch_csv_preprocess }

process preprocessSingleCellData {

    publishDir "outputs/sce", 
        pattern: "*.rda", 
        saveAs: {filename -> (new File(filename).name)},
        mode: "copy"

    input:
        path("inputs/dataset*") from Ch_cell_data.collect()
        path("inputs/cells*") from Ch_report_cells.collect()
        file(csv) from Ch_csv_preprocess

    output:
        file("sce.rda") into (Ch_basic_sce, Ch_de_barcodes_sce)

    script:
    """
    touch sampleslist
    for DIR in \$(find inputs -mindepth 1 -maxdepth 1 -name "dataset*")
    do
       FULL_PATH=\$(readlink -f \${DIR})
       SAMPLE=\$(echo \${FULL_PATH} | rev | cut -d"/" -f2 | rev)
       echo -e "\${SAMPLE},\${FULL_PATH}" >> sampleslist
    done

    touch barcodes
    for DIR in \$(find inputs -mindepth 1 -maxdepth 1 -name "cells*")
    do
       FULL_PATH=\$(readlink -f \${DIR})
       SAMPLE=\$(basename \${FULL_PATH} .cells)
       echo -e "\${SAMPLE},\${FULL_PATH}" >> barcodes
    done

    join -j 1 <(sort sampleslist) <(sort barcodes) -t "," > samplesdata

    #Rscript --vanilla /tools/scripts/R/preprocessData.R \
    Rscript --vanilla /tools/scripts/R/preprocessData.R
       --samples samplesdata \
       --csv ${csv} \
       --reference ${params.refName} \
