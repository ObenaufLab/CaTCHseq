...
workflow{
    main:
        ...
        ***********************************************************/
        ...
        if (mapperbin == 'CellRanger'){
            Ch_map_input = Ch_csv_GEX_split.raw.map { row -> tuple(row.SampleName.replaceAll("_","-")+'_'+row.Condition.replaceAll("_","-")+'_'+row.Replicate.replaceAll("_","-"), file(row.R1), file(row.R2), row.CellNumber.toString(), row.Chemistry.toString() ) }.groupTuple(by: 0).combine( Ch_mapping_idx )
            ...
            Ch_map_input = Ch_csv_GEX_split.raw.map { row -> tuple(row.SampleName.replaceAll("_","-")+'_'+row.Treatment.replaceAll("_","-")+'_'+row.Replicate.replaceAll("_","-"), file(row.R1), file(row.R2), row.CellNumber.toString(), row.Chemistry.toString() ) }.groupTuple(by: 0).combine(Ch_mapping_idx)
        }
